...
void rcvWaypointsCallBack(const nav_msgs::Path& wp) {
  ...
  for (int k = 0; k < (int)wp.poses.size(); k++) {
    Vector3d pt(wp.poses[k].pose.position.x, wp.poses[k].pose.position.y,
...
void trajGeneration(const std::vector<Eigen::Vector3d> & pos) {
  ...
  // give an arbitraty time allocation, all set all durations as 1 in the
  // commented function.
  ...
  _polyCoeff = trajectoryGeneratorWaypoint.PolyQPGeneration(
      _dev_order, path, vel, acc, _polyTime);

  visWayPointPath(path);

  // After you finish your homework, you can use the function visWayPointTraj
  // below to visulize your trajectory visWayPointTraj( _polyCoeff, _polyTime);
}
...
VectorXd timeAllocation(MatrixXd Path) {
  ...
  STEP 1: Learn the "trapezoidal velocity" of "TIme Allocation" in L5, then
  finish this timeAllocation function

  variable declaration: _Vel, _Acc: _Vel = 1.0, _Acc = 1.0 in this homework, you
  can change these in the test.launch

  You need to return a variable "time" contains time allocation, which's type is
  VectorXd
